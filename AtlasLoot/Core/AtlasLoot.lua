...
function AtlasLoot_ShowItemsFrame(dataID, dataSource, boss, pFrame)
	...
	elseif(dataID=="ZGSET") then
		AtlasLootZGSetMenu();
	elseif(dataID=="T8SET") then
		AtlasLootT8SetMenu();
	elseif(dataID=="TXSET") then
		AtlasLootTXSetMenu();
	...
	elseif(dataID=="SPELLSMENU") then
		AtlasLootSpellsMenu();
	elseif(dataID=="BUFFSMENU") then
		AtlasLootBuffsMenu();
	elseif(dataID=="PROFESSIONSMENU") then
		AtlasLootProfessionsMenu();
	elseif(dataID=="SPELLS") then
		AtlasLootSPELLS();
	elseif(dataID=="BUFFS") then
		AtlasLootBuffs();
	elseif(dataID=="PROFESSIONS") then
		AtlasLootProfessions();
	else
		...
		for i = 1, 30, 1 do
			...
			if(dataSource[dataID][i] ~= nil and dataSource[dataID][i][3] ~= "") then
				...
				elseif dataSource[dataID][i][2] == "" then
					iconFrame:SetTexture(GetItemIcon(dataSource[dataID][i][1]) or select(10,GetItemInfo(dataSource[dataID][i][1])));
				elseif (not isItem) and (spellIcon) then
					if type(dataSource[dataID][i][2]) == "number" then
						iconFrame:SetTexture(GetItemIcon(dataSource[dataID][i][2]) or select(10,GetItemInfo(dataSource[dataID][i][2])));
...
function AtlasLoot_GetLODModule(dataSource)
	...
	elseif (dataSource=="AtlasLootSpells") then
		return "AtlasLoot_Spells";
...
function AtlasLoot_LoadAllModules()
	...
	if not AtlasLoot_Data["AtlasLootSpells"] then
		LoadAddOn("AtlasLoot_Spells");
		flag=1;
	end
